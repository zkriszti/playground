<template>
  ...
  <h1>Daily schedule planner</h1>
    <div class="input-fields">
      ...
      <input type="number" id="startTime" name="startTime" min="8" max="23" v-model="modifiedQueueStart">

      <label for="closeTime">End time period at: (9-24)</label>
      <input type="number" id="closeTime" name="startTime" min="9" max="24" v-model="modifiedQueueClose">
    ...
    <button @click="changeSchedule(modifiedQueueStart, modifiedQueueClose, modifiedStatus)" :disabled="inputError">Update timeline and summary</button>
    <div class="schedule">    
  ...
  computed: {
    displaySummary () {
      ...
      const result = this.splitSummary.map(split => {
        return split.map(splitItem => {
          const newItem = {itemStart: currentItemPosition + this.earliestStartTime, status: splitItem}
          currentItemPosition++
          return newItem
        })
      })
    ...
    totalOpenHours () {
      if (this.displaySummary.length === 0) {
        // no summary is created yet, initial state
        return this.schedule.filter(item => item).length
      } else {
